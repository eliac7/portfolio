...
export default function Experience() {
  ...
  const visibleExperiences = experiencesData.slice(0, 6);
  const earlierExperiences = experiencesData.slice(6);

  const renderTimelineItem = (item: (typeof experiencesData)[number], index: number) => (
    <motion.li
      key={item.id}
  ...
  return (
    <section id="experience" ref={ref} className="mb-28 w-full max-w-5xl scroll-mt-28 sm:mb-40">
      ...
      <ol className="relative ml-3 border-l border-slate-300 pl-7 dark:border-white/15 sm:ml-0 sm:pl-10">
        {visibleExperiences.map(renderTimelineItem)}
        {earlierExperiences.length > 0 && (
          <li className="relative">
            ...
            <details className="group">
              <summary className="flex min-h-14 list-none cursor-pointer items-center justify-between gap-4 rounded-3xl border border-slate-200 bg-white/70 px-5 py-4 text-sm font-semibold text-slate-950 shadow-sm outline-none transition-colors hover:border-indigo-300 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-indigo-500 dark:border-white/10 dark:bg-white/[0.04] dark:text-white dark:hover:border-indigo-300/60 sm:px-7">
                <span>Earlier experience &amp; background</span>
                <span className="shrink-0 text-xs font-medium text-slate-500 dark:text-slate-400">
                  {earlierExperiences.length} entries
                </span>
              </summary>
              <ol className="mt-8">
                {earlierExperiences.map((item, index) => renderTimelineItem(item, index + visibleExperiences.length))}
              </ol>
            </details>
          </li>
        )}
